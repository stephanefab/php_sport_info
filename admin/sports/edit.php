...
if (isset($_POST['submit_form'])) {
    ...
    if (!hasErrors()) {
        ...
        $exist_name = getOneByColumn("sports", "name", $name);
        if($exist_name && $exist_name['id'] != $id){
            setFlashMessage("Le nom '$name' est déjà utilisé", "error");
        }
        
        if (!hasErrors()){
             $updated = update("sports", $id, [
                "name"     => $name,
                "path"     => $path,
            ]);

            if ($updated) {
                setFlashMessage("sport modifié: $name", "success");
            } else {
...
<div class="container my-4">
     <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold mb-4">Modifier le sport : <?= $name ?></h2>
        <a href="/admin/sports#<?= $id ?>" class="btn btn-dark">
            Retour
        </a>
    </div>
