...
if (isset($_POST['submit_form'])) {
    ...
    if (!hasErrors()) {
        ...
        if (!hasErrors()) {
            ...
            if ($updated) {
                setFlashMessage("équipe modifiée: $name", "success");
            } else {
...
<div class="container my-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold mb-4">Modifier l'équipe : <?= $name ?></h2>
        <a href="/admin/teams#<?= $id ?>" class="btn btn-dark">
            Retour
        </a>
    </div>
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="type" class="form-control" id="type">
                ...
                <option class="form-control" <?= (isset($type) && $type === "national") ? "selected" : '' ?>
                    value="national">National</option>
                <option class="form-control" <?= (isset($type) && $type === "club") ? "selected" : '' ?> value="club">Club
                </option>
            </select>
        </div>
